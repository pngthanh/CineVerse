...
export function SeatSelectionPage() {
    ...
    const holdSeats = async () => {
        ...
        try {
            ...
            setError(exception instanceof ApiError ? exception.message : 'Không thể giữ ghế. Vui lòng thử lại.');
            setSelectedSeatIds([]);
    ...
    const maxSeatsPerRow = useMemo(
        () => Math.max(0, ...rows.map(([, seats]) => seats.length)),
        [rows],
    );
    const aisleAfter = Math.max(1, Math.ceil(maxSeatsPerRow / 2));
    const leftSeats = aisleAfter;
    const rightSeats = Math.max(0, maxSeatsPerRow - aisleAfter);
    const gridTemplateColumns = `30px repeat(${leftSeats}, 44px) 30px repeat(${rightSeats}, 44px)`;
    const gridColumnForSeat = (seatNumber: number) => seatNumber <= aisleAfter ? seatNumber + 1 : seatNumber + 2;
    const seatCanvasWidth = Math.max(620, 30 + maxSeatsPerRow * 51 + 44);
        ...
        <div className="container page">
            ...
            <div className="seat-layout">
                <section className="seat-panel">
                    ...
                    <div className="seat-map-scroll" aria-label="Sơ đồ ghế có thể cuộn ngang trên màn hình nhỏ">
                        <div
                            className="seat-grid-stage"
                            style={{
                                gridTemplateRows: `repeat(${Math.max(1, rows.length)}, 34px)`,
                                minWidth: `${seatCanvasWidth}px`,
                            }}
                        >
                            {rows.map(([row, seats], rowIndex) => (
                                <div
                                    ...
                                    key={row}
                                    style={{ gridRow: rowIndex + 1, gridTemplateColumns }}
                                >
                                    ...
                                    {seats.map((seat) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span>{seat.code}</span>
                                                {seat.status === 'BOOKED' && <span className="seat-booked-x" aria-hidden="true">×</span>}
                                            </button>
                                        );
                                    })}
                                </div>
                    ...
                    <div className="aisle-legend">
                        Lối đi nằm giữa phòng · Khu VIP là phần lõi trung tâm, cách mép trên/dưới/trái/phải 2 hàng hoặc cột
                    </div>
                </section>
                    ...
                    <div className="selected-list">
                        {selectedSeats.length ? selectedSeats.map((seat) => (
                            <div className="selected-seat-row" key={seat.seatId}>
                                <span className="selected-seat-name">{seat.code} · {seatTypeLabel(seat.type)}</span>
                                <strong className="selected-seat-price">{money(seat.price)}</strong>
                            </div>
                        )) : <p>Chưa chọn ghế.</p>}
                    </div>
                    <div className="summary-row total"><span>Tạm tính</span><strong>{money(total)}</strong></div>
                    <button disabled={!selectedSeatIds.length || holding} className="btn btn-block" onClick={holdSeats}>
