                ...
                <div className="admin-stack">
                    <form className="panel" onSubmit={submitCinema}>
                        ...
                        <label>
                            ...
                            <input
                                ...
                                onChange={(event) => setCinemaForm((value) => ({ ...value, name: event.target.value }))}
                                required
                            />
                        </label>
                        <label>
                            ...
                            <input
                                ...
                                onChange={(event) => setCinemaForm((value) => ({ ...value, address: event.target.value }))}
                                required
                            />
                        </label>
                        <label className="admin-checkbox-row">
                            <input
                                ...
                                onChange={(event) => setCinemaForm((value) => ({ ...value, active: event.target.checked }))}
                            />
                            Rạp đang hoạt động
                        </label>
                    ...
                    <form className="panel" onSubmit={submitRoom}>
                        ...
                        <label>
                            ...
                            <input
                                value={roomForm.name}
                                onChange={(event) => setRoomForm((value) => ({ ...value, name: event.target.value }))}
                                required
                            />
                        </label>
                        <div className="form-two-col">
                            <label>
                                ...
                                <input
                                    ...
                                    max="26"
                                    value={roomForm.rows}
                                    onChange={(event) => setRoomForm((value) => ({ ...value, rows: Number(event.target.value) }))}
                                    required
                                />
                            </label>
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setRoomForm((value) => ({ ...value, seatsPerRow: Number(event.target.value) }))}
                                    required
                                />
                            </label>
                        </div>
                        <div className="room-price-grid">
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setRoomForm((value) => ({ ...value, weekdayBasePrice: Number(event.target.value) }))}
                                    required
                                />
                            </label>
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setRoomForm((value) => ({ ...value, weekendBasePrice: Number(event.target.value) }))}
                                    required
                                />
                            </label>
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(event) => setRoomForm((value) => ({ ...value, vipSurcharge: Number(event.target.value) }))}
                                    required
                                />
                            </label>
                        </div>
                        <label className="admin-checkbox-row">
                            <input
                                ...
                                onChange={(event) => setRoomForm((value) => ({ ...value, active: event.target.checked }))}
                            />
                            Phòng đang hoạt động
                        </label>
                        <div className="vip-rule-note">
                            <strong>Quy tắc VIP tự động</strong>
                            <span>
                                Tối thiểu 6×6. Chừa 2 hàng trên/dưới và 2 cột trái/phải là ghế thường;
                                toàn bộ lõi ở giữa là VIP.
                            </span>
                        </div>
                        {editingRoomId && (
                            ...
                            >
                                <div className="cinema-admin-card-head">
                                    ...
                                    <div className="admin-inline-actions">
                                        <button className="btn btn-secondary btn-sm" type="button" onClick={() => editCinema(cinema)}>
                                            Sửa rạp
                                        </button>
                                        {cinema.active && (
                                            <button className="btn btn-danger btn-sm" type="button" onClick={() => void deactivateCinema(cinema)}>
                                                Xóa / ngừng
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className="cinema-admin-summary">
                                    ...
                                    <span>{cinema.rooms.reduce((total, room) => total + room.vipSeatCount, 0)} VIP</span>
                                </div>

                                <div className="admin-room-list">
                                    {cinema.rooms.map((room) => (
                                        <div className={`admin-room-card ${!room.active ? 'is-inactive' : ''}`} key={room.id}>
                                            <div className="admin-room-main">
                                                <div>
                                                    <strong>{room.name}</strong>
                                                    <p>{room.rows} hàng × {room.seatsPerRow} ghế · {room.vipSeatCount} VIP</p>
                                                </div>
                                                <span className={room.active ? 'room-status-active' : 'room-status-inactive'}>
                                                    {room.active ? 'Hoạt động' : 'Ngừng'}
                                                </span>
                                            </div>
                                            <div className="admin-room-prices">
                                                <span><small>Ngày thường</small><strong>{money(room.weekdayBasePrice)}</strong></span>
                                                <span><small>Cuối tuần</small><strong>{money(room.weekendBasePrice)}</strong></span>
                                                <span><small>VIP cộng</small><strong>+{money(room.vipSurcharge)}</strong></span>
                                            </div>
                                            <div className="admin-inline-actions room-actions">
                                                <button className="btn btn-secondary btn-sm" type="button" onClick={() => editRoom(cinema, room)}>
                                                    Sửa phòng
                                                </button>
                                                {room.active && (
                                                    <button className="btn btn-danger btn-sm" type="button" onClick={() => void deactivateRoom(room)}>
                                                        Xóa / ngừng
                                                    </button>
                                                )}
                                            </div>
                                    ...
                                    {cinema.rooms.length === 0 && <div className="empty">Rạp chưa có phòng chiếu.</div>}
                                </div>
                            </article>
                        ))}
