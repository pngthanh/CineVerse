...
type CinemaForm = { name: string; address: string; active: boolean };
type RoomForm = {
    name: string;
    rows: number;
    seatsPerRow: number;
...
export function AdminCinemasPage() {
    ...
    const openCreateCinema = () => {
        setEditingCinemaId(null); setCinemaForm(emptyCinema); setError(''); setCinemaModalOpen(true);
    };
    ...
    const submitCinema = async (event: FormEvent) => {
        ...
        try {
            if (editingCinemaId) {
                await api(`/admin/cinemas/${editingCinemaId}`, { method: 'PUT', body: JSON.stringify(cinemaForm) });
                setMessage('Đã cập nhật rạp.');
            } else {
                await api('/admin/cinemas', { method: 'POST', body: JSON.stringify(cinemaForm) });
    ...
    return <div className="admin-page">
        ...
        <section className="panel cinema-admin-list-panel admin-full-panel">
            ...
            <div className="cinema-admin-list">{cinemas.map((cinema) => <article className={`cinema-admin-card ${selectedCinema?.id === cinema.id ? 'selected' : ''} ${!cinema.active ? 'is-inactive' : ''}`} key={cinema.id}>
                <div className="cinema-admin-card-head">
                    ...
                    <div className="admin-inline-actions"><button className="btn btn-secondary btn-sm" type="button" onClick={() => editCinema(cinema)}>Sửa rạp</button>{cinema.active && <button className="btn btn-danger btn-sm" type="button" onClick={() => void deactivateCinema(cinema)}>Xóa / ngừng</button>}</div>
                </div>
                <div className="cinema-admin-summary"><span>{cinema.rooms.length} phòng</span><span>{cinema.rooms.reduce((total, room) => total + room.seatCount, 0)} ghế</span><span>{cinema.rooms.reduce((total, room) => total + room.vipSeatCount, 0)} VIP</span></div>
                <div className="admin-room-list">{cinema.rooms.map((room) => <div className={`admin-room-card ${!room.active ? 'is-inactive' : ''}`} key={room.id}>
                    ...
                    <div className="admin-room-prices"><span><small>Ngày thường</small><strong>{money(room.weekdayBasePrice)}</strong></span><span><small>Cuối tuần</small><strong>{money(room.weekendBasePrice)}</strong></span><span><small>VIP cộng</small><strong>+{money(room.vipSurcharge)}</strong></span></div>
                    <div className="admin-inline-actions room-actions"><button className="btn btn-secondary btn-sm" type="button" onClick={() => editRoom(cinema, room)}>Sửa phòng</button>{room.active && <button className="btn btn-danger btn-sm" type="button" onClick={() => void deactivateRoom(room)}>Xóa / ngừng</button>}</div>
        ...
        <Modal open={cinemaModalOpen} title={editingCinemaId ? 'Chỉnh sửa rạp' : 'Thêm rạp'} onClose={() => setCinemaModalOpen(false)}>
            ...
            <form className="modal-form" onSubmit={submitCinema}><label>Tên rạp<input value={cinemaForm.name} onChange={(e) => setCinemaForm((value) => ({ ...value, name: e.target.value }))} required /></label><label>Địa chỉ<textarea value={cinemaForm.address} onChange={(e) => setCinemaForm((value) => ({ ...value, address: e.target.value }))} required /></label><label className="admin-checkbox-row"><input type="checkbox" checked={cinemaForm.active} onChange={(e) => setCinemaForm((value) => ({ ...value, active: e.target.checked }))} />Rạp đang hoạt động</label><div className="modal-actions"><button className="btn btn-secondary" type="button" onClick={() => setCinemaModalOpen(false)}>Hủy</button><button className="btn">{editingCinemaId ? 'Lưu thay đổi' : 'Tạo rạp'}</button></div></form>
        </Modal>
