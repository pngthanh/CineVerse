...
function readLocalResumeTarget(): ResumeTarget {
  try {
    const booking = JSON.parse(
      sessionStorage.getItem('cineverse_booking') || 'null',
    ) as Booking | null;
    ...
    <header className="site-header">
      <div className="container nav">
        ...
        {user && resumeTarget && location.pathname !== resumeTarget.path && (
          <div className="nav-resume-slot">
            <button
              ...
              type="button"
              onClick={() => navigate(resumeTarget.path)}
            >
        ...
        <div className="nav-actions">
          ...
          {user?.role === 'STAFF' && (
            <NavLink className="nav-admin" to="/staff">
              Trang nhân viên
            </NavLink>
          )}

          <NavLink className="nav-ticket" to="/bookings">
            Vé của tôi
          </NavLink>
