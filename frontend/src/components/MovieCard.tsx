  ...
  return (
    <article className="movie-card">
      ...
      >
        ...
        <span className="movie-score">★ {(movie.ratingAverage ?? 0).toFixed(1)}</span>
      </Link>
      <div className="movie-info">
        <div className="movie-title-slot">
          <h3>{movie.title}</h3>
        </div>
        <div className="movie-meta-slot">
          <p>{movie.genres}</p>
          <p>
            {movie.durationMinutes} phút · {movie.reviewCount ?? 0} đánh giá
          </p>
        </div>
        <div className="movie-action-slot">
          {movie.status === 'NOW_SHOWING' ? (
            <Link className="btn btn-block" to={`/movies/${movie.id}`}>
              Đặt vé
            </Link>
          ) : (
            <Link className="btn btn-secondary btn-block" to={`/movies/${movie.id}`}>
              Xem chi tiết
            </Link>
          )}
        </div>
